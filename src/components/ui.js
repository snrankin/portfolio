...
export function Base({ as: Component = 'div', cx: _cx = [], className, ...props }) {
...
export function Flex({ variant, gap = 3, gutter, wrap, responsive, marginY, alignItems, cx: _cx = [], ...props }) {
	return (
		<Base
			cx={[
				styles.flex,
...
export function Text({ variant = 'body', center = false, bold = false, ...props }) {
	return <Base cx={[styles.text[variant], center && styles.text.center, bold && styles.text.bold]} {...props} />;
}
...
export function Avatar({ alt, image }) {
	return <GatsbyImage alt={alt} image={getImage(image)} className={styles.avatar} />;
}

export function Logo({ alt, image, size = 'small' }) {
	return <GatsbyImage alt={alt} image={getImage(image)} className={styles.logos[size]} />;
}

export function Icon({ alt, image, size = 'medium' }) {
	return <GatsbyImage alt={alt} image={getImage(image)} className={styles.icons[size]} />;
}
